...
static void worker_destructor(struct llhead *lp)
{
	...
	free_list(&worker->connections, connection_destructor);
	free(worker);
}
...
#if 0
	if (!(c->loc.flags & FLAG_HEADERS_PARSED)) {
		const char* cp = strstr(c->loc.io.buf, "\r\n\r\n");
		if (cp) {
			c->loc.flags |= FLAG_HEADERS_PARSED;
			MY_DEBUG( "sock:%d:%d, loc:%s:%d, [%.*s]\n",
			c->rem.chan.sock, c->rem.io.total,
			c->loc.io_class ? c->loc.io_class->name : "nil", c->loc.io.total,
			cp - c->loc.io.buf, c->loc.io.buf);
		}
	}
	MY_DEBUG("loc: %d [%.*s]\n", (int) io_data_len(&c->loc.io),
	    (int) io_data_len(&c->loc.io), io_data(&c->loc.io));
	MY_DEBUG("rem: %d [%.*s]\n", (int) io_data_len(&c->rem.io),
	    (int) io_data_len(&c->rem.io), io_data(&c->rem.io));

	if (c->current_time != _shttpd_current_time) {
		c->current_time = _shttpd_current_time;
		MY_DEBUG( "sock:%d:%d, loc:%s:%d, t:%d,%d.\n",
			c->rem.chan.sock, c->rem.io.total,
			c->loc.io_class ? c->loc.io_class->name : "nil", c->loc.io.total,
			_shttpd_current_time - c->birth_time,
			c->expire_time - _shttpd_current_time);
	}
#endif
...
//		MY_DEBUG("sock:%d, %d.\n", c->rem.chan.sock, io_data_len(&c->loc.io));
		...
		struct usa *sap, int sock, int is_ssl)
{
	...
	if (sock >= (int) FD_SETSIZE) {
		MY_ERROR("ctx %p: discarding socket %d, too busy\n", ctx, sock);
		(void) closesocket(sock);
	} else
#endif /* !_WIN32 */
		if (!is_allowed(ctx, sap)) {
		MY_ERROR("%s is not allowed to connect\n", inet_ntoa(sap->u.sin.sin_addr));
		(void) closesocket(sock);
...
static int do_select(int max_fd, fd_set *read_set, fd_set *write_set, int milliseconds)
{
	...
	if ((n = select(max_fd + 1, read_set, write_set, NULL, &tv)) < 0) {
		...
		Sleep(milliseconds);
#endif /* _WIN32 */
		// wkliang: MY_DEBUG("select: n=%d, errno=%d\n", n, ERRNO);
	}
		...
		fd_set *read_set, fd_set *write_set, int *max_fd)
{
	...
	LL_FOREACH(&ctx->listeners, lp) {
		...
		// wkliang: MY_DEBUG("FD_SET(%d) (listening)\n", l->sock);
	}
...
#define NOWAIT_MILLISEC	100	/* wkliang:20110525 do not (nowait); */
/*
...
void shttpd_poll(struct shttpd_ctx *ctx, int milliseconds)
{
	...
	if (shttpd_join(ctx, &read_set, &write_set, &max_fd))
		milliseconds = NOWAIT_MILLISEC; // wkliang:20110525
...
static int set_acl(struct shttpd_ctx *ctx, const char *s)
{
	...
	FOR_EACH_WORD_IN_LIST(s, len) {
		...
		if (sscanf(s, "%c%d.%d.%d.%d%n",&flag,&a,&b,&c,&d,&n) != 5) {
			MY_ABORT("[%s]: subnet must be [+|-]x.x.x.x[/x]\n", s);
		} else if (flag != '+' && flag != '-') {
			MY_ABORT("flag must be + or -: [%s]\n", s);
		} else if (!isbyte(a)||!isbyte(b)||!isbyte(c)||!isbyte(d)) {
			MY_ABORT("bad ip address: [%s]", s);
		} else	if ((acl = malloc(sizeof(*acl))) == NULL) {
			MY_ABORT("%s", "cannot malloc subnet\n");
		} else if (sscanf(s + n, "/%d", &mask) == 0) { 
			/* Do nothing, no mask specified */
		} else if (mask < 0 || mask > 32) {
			MY_ABORT("bad subnet mask: %d [%s]\n", n, s);
		}
...
static int open_log_file(FILE **fpp, const char *path)
{
	...
	if ((path == NULL) || (*path == '\0')) {
		*fpp = stderr;	/* NULL */
		...
		MY_ERROR("cannot open log file %s: %s\n",
		    path, strerror(errno));
		retval = FALSE;
	}
...
static struct worker* add_worker(struct shttpd_ctx *ctx)
{
	...
	if ((worker = calloc(1, sizeof(*worker))) == NULL)
		MY_ABORT("Cannot allocate worker\n");
...
static void worker_function(void *param)
{
	...
	MY_DEBUG("Enter worker_function.\n");
	while (worker->exit_flag == 0)
		poll_worker(worker, 1000 * 10);

	free_list(&worker->connections, connection_destructor);
	free(worker);
	MY_DEBUG("Leave worker_function.\n");
}
...
/* wkliang:20081216 - disable builtin systray */
	{OPT_HIDE, "systray", "Hide console, show icon on systray",
		"no", _shttpd_set_systray},
#endif /* _WIN32 */
	{OPT_ACCESS_LOG, "access_log", "Access log file", "", set_alog},
	{OPT_ERROR_LOG, "error_log", "Error log file", "", set_elog},
...
#if !defined(NO_THREADS)
	{OPT_THREADS, "threads", "Number of worker threads", "3", set_workers},
#endif /* !NO_THREADS */
...
static void show_cfg_page(struct shttpd_arg *arg)
{
	...
	if (!strcmp(shttpd_getenv(arg, "REQUEST_METHOD"), "POST")) {
	...
	for (o = known_options; o->name != NULL; o++) {
		shttpd_printf(arg,
		    "<tr><form method=post><td>%s</td><td>%s</td>"
...
static void process_command_line_arguments(struct shttpd_ctx *ctx, char *argv[])
{
	...
	if (fp == NULL && argv[i] != NULL)
		MY_ABORT("cannot open config file %s: %s\n",
		    config_file, strerror(errno));

	if (fp != NULL) {
		MY_ERROR("Loading config file %s\n", config_file);
		...
		while (fgets(line, sizeof(line), fp) != NULL) {
			...
			if (sscanf(line, "%s %[^\n#]", opt, val) != 2)
				MY_ABORT("line %d in %s is invalid\n",
				    line_no, config_file);
...
struct shttpd_ctx* shttpd_init(int argc, char *argv[])
{
	...
	if ((ctx = calloc(1, sizeof(*ctx))) == NULL)
		MY_ABORT("cannot allocate shttpd context\n");
	...
	for (o = known_options; o->name != NULL; o++) {
		...
		MY_DEBUG("options[%d]:%s:%s\n", o->index, o->name,
 			ctx->options[o->index] ?  ctx->options[o->index] : "NULL");
	}
