...
static int evaluate(struct ssi *ssi, const char *name)
{
	...
	// wkliang:20110527 - a bit tricky & risky, eval logic cannot output
...
again:
	...
	return (n); // wkliang:20110530 read_stream() get 0 stop stream
}
...
static void close_ssi(struct stream *stream)
{
	...
	MY_DEBUG("stream:%p\n", stream);
	MY_DEBUG("stream->conn:%p\n", stream->conn);
	MY_DEBUG("stream->conn->ssi:%p\n", stream->conn->ssi);
	if (ssi) { // wkliang:20110526 - check nil condition
...
void _shttpd_do_ssi(struct conn *c)
{
	...
	c->loc.flags |= FLAG_R | FLAG_ALWAYS_READY; // wkliang:20110530: only GET?

	if (c->method != METHOD_GET) { // wkliang:20110530 org: == METHOD_HEAD
